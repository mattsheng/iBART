...
#' @param hold Number of iterations to hold. This allows iBART to run consecutive operator transformations before screening. Note \code{hold = 1} is equivalent to no skipping of variable selection in each iBART iterations. It should be less than \code{iter}.
...
iBART <- function(X = NULL, y = NULL,
                  ...
                  nreps = 2000,
                  top = (2000 * 0.05 * 2), # Top 5% of trees
                  perm = FALSE,
                  opt = 2,
                  sin_cos = FALSE,
  ...
  if (standardize == TRUE) {
    ...
    if (out_sample == TRUE){
      X_test <- sapply(1:ncol(X_test), function(k) (X_test[, k] - train_col_mean[k]) / train_col_sd[k])
  ...
  if (Lzero == TRUE){
    ...
    if (p > 1) {
      ...
      for (k in 1:min(p, K)) {
        k_model <- k_var_model(X_train, y_train, X_test, y_test, k, parallel = FALSE) # parallel doesn't work well yet
      ...
      if (AIC == TRUE) {
        ...
        aic_rmse_out <- rmse_k_model_out[lowest_aic_idx]
        aic_rmse_in <- rmse_k_model_in[lowest_aic_idx]
      }
    } else {
      cat("iBART only selected 1 descriptor, skipping L-zero regression... \n")
      rmse_k_model_out <- aic_rmse_out <- if (out_sample) LASSO_selection$Out_sample_RMSE else NA
      rmse_k_model_in <- aic_rmse_in <- LASSO_selection$In_sample_RMSE
      Lzero_models <- aic_model <- LASSO_selection$LASSO_model
  ...
  iBART_output <- list(iBART_model = LASSO_selection$LASSO_model,
                       ...
                       Lzero_aic_in_sample_RMSE = if (Lzero && AIC) aic_rmse_in else NA,
                       Lzero_aic_out_sample_RMSE = if (out_sample && Lzero && AIC) aic_rmse_out else NA,
                       runtime = runtime)
  return(iBART_output)
}
